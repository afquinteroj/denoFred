...
export default class SeriesFred extends FRED_API {
    ...
    /*async seriesFetch(series_id, request='series', latentOptions='', filename='fredData.zip') {
        try {

            if(!request.includes('series')) {
                throw Error(`Warning, request not set correctly.`)
            };
            if(typeof series_id === 'undefined') {
                throw Error(`Warning, series not set.`)
            }; 

            const finalOptions = latentOptions === '' ? this.optionsURL : setOptions(latentOptions);
            const requestPoint = request.concat('/');
            const seriesArg = 'series_id='.concat(series_id);

            this.#fullURL = this.baseURL.concat(requestPoint, seriesArg, this.#apiKey, finalOptions)
