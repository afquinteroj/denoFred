...
export default class CategoryFred extends FRED_API {
    ...
    async fetchCategoryRelatedTags(category_id, tag_names) {
        try {
            if(typeof tag_names === 'undefined' || typeof category_id === 'undefined') {
                throw Error(`Warning, mandatory arguments not set.`)
            };
            
            // Fix below later. Below should be a method from parent function

            const search_string = '&tag_names='.concat(tag_names.join(';'));
            this.#fullURL = this.baseURL.concat('category/related_tags?category_id=', category_id, search_string, this.optionsURL, this.#apiKey);
            const data = await super.fetchData(this.#fullURL, this.options?.file_type);
