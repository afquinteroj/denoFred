export default class FRED_API {
    ...
    constructSearchUrl(search_array, endPoint, baseURL, apiKey, optionsURL) {
        const search_string = search_array.join('+');
        return baseURL.concat(endPoint, search_string, apiKey, optionsURL);
    };
    
    constructRelatedTagSearchUrl(search_array, tag_names, endPoint, baseURL, apiKey, optionsURL) {
        const search_string = search_array.join('+');
        const tag_string = '&tag_names='.concat(tag_names.join(';'));
        const search_params = search_string.concat(tag_string);
        return baseURL.concat(endPoint, search_params, apiKey, optionsURL);
    };
